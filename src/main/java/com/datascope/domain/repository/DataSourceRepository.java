...
public interface DataSourceRepository
    extends JpaRepository<DataSource, String>, JpaSpecificationExecutor<DataSource> {
  ...
  /** 分页查询数据源 */
  @Query(
      "SELECT ds FROM DataSource ds WHERE "
  ...
  /** 查找指定时间段内创建的数据源 */
  @Query(
      "SELECT ds FROM DataSource ds WHERE ds.createdAt >= :startTime AND ds.createdAt <= :endTime")
  List<DataSource> findByCreatedTimeBetween(
      @Param("startTime") String startTime, @Param("endTime") String endTime);

  /** 查找最近修改的数据源 */
  @Query("SELECT ds FROM DataSource ds ORDER BY ds.updatedAt DESC")
