...
public class DataPreviewServiceImpl implements DataPreviewService {
    ...
    public DataPreviewResponse previewData(DataPreviewRequest request) {
        ...
        try {
            ...
            if (dataSource == null) {
                throw new IllegalArgumentException("Data source not found: " + request.getDataSourceId());
            }

            // 获取表元数据
            TableMetadata tableMetadata = metadataStorageService.getTableMetadata(
                request.getDataSourceId(), request.getSchema(), request.getTableName());
            if (tableMetadata == null) {
                throw new IllegalArgumentException("Table not found: " + request.getSchema() + "." + request.getTableName());
            }
    ...
    private Long getTotalRows(Connection conn, DataPreviewRequest request, TableMetadata tableMetadata) {
        StringBuilder sql = new StringBuilder();
